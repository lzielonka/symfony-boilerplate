...
use \Exception;
use \PDO;
use ModelBundle\Propel\Model\Account as ChildAccount;
 ...
 *
 * @method     ChildAccount[]|\ModelBundle\Propel\Collection\ObjectCollection find(ConnectionInterface $con = null) Return ChildAccount objects based on current ModelCriteria
 * @method     ChildAccount[]|\ModelBundle\Propel\Collection\ObjectCollection findByAccountId(int $account_id) Return ChildAccount objects filtered by the account_id column
 * @method     ChildAccount[]|\ModelBundle\Propel\Collection\ObjectCollection findByEmail(string $email) Return ChildAccount objects filtered by the email column
 * @method     ChildAccount[]|\ModelBundle\Propel\Collection\ObjectCollection findByPasswordHash(string $password_hash) Return ChildAccount objects filtered by the password_hash column
...
abstract class AccountQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \ModelBundle\Propel\Model\Base\AccountQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\ModelBundle\\Propel\\Model\\Account', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT account_id, email, password_hash FROM account WHERE account_id = :p0';
        try {
