...
/**
 * Base class that represents a row from the 'account_book' table.
 *
 *
 *
 * @package    propel.generator.src.AppBundle.Database.Propel.Model.Base
 */
abstract class AccountBook implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\AppBundle\\Database\\Propel\\Model\\Map\\AccountBookTableMap';
    ...
    /**
     * Initializes internal state of AppBundle\Database\Propel\Model\Base\AccountBook object.
     */
    ...
    /**
     * Compares this with another <code>AccountBook</code> instance.  If
     * <code>obj</code> is an instance of <code>AccountBook</code>, delegates to
     * <code>equals(AccountBook)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|AccountBook The current object, for fluid interface
     */
     ...
     * @return $this|\AppBundle\Database\Propel\Model\AccountBook The current object (for fluent API support)
     */
    public function setAccountId($v)
    {
        ...
        if ($this->account_id !== $v) {
            $this->account_id = $v;
            $this->modifiedColumns[AccountBookTableMap::COL_ACCOUNT_ID] = true;
        }
    ...
    public function setBookId($v)
    {
        ...
        if ($this->book_id !== $v) {
            $this->book_id = $v;
            $this->modifiedColumns[AccountBookTableMap::COL_BOOK_ID] = true;
        }

        if ($this->aBook !== null && $this->aBook->getBookId() !== $v) {
            $this->aBook = null;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : AccountBookTableMap::translateFieldName('AccountId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->account_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : AccountBookTableMap::translateFieldName('BookId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = AccountBookTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\AppBundle\\Database\\Propel\\Model\\AccountBook'), 0, $e);
        }
    }
    ...
    public function ensureConsistency()
    {
        ...
        if ($this->aBook !== null && $this->book_id !== $this->aBook->getBookId()) {
            $this->aBook = null;
        }
    } // ensureConsistency
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        if ($con === null) {
            $con = Propel::getServiceContainer()->getReadConnection(AccountBookTableMap::DATABASE_NAME);
        }

        // We don't need to alter the object instance pool; we're just modifying this instance
        // already in the pool.

        $dataFetcher = ChildAccountBookQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see AccountBook::setDeleted()
     * @see AccountBook::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildAccountBookQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                AccountBookTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(AccountBookTableMap::COL_ACCOUNT_ID)) {
            $modifiedColumns[':p' . $index++]  = 'account_id';
        }
        if ($this->isColumnModified(AccountBookTableMap::COL_BOOK_ID)) {
            $modifiedColumns[':p' . $index++]  = 'book_id';
        }

        $sql = sprintf(
            'INSERT INTO account_book (%s) VALUES (%s)',
    ...
    {
        $pos = AccountBookTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['AccountBook'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['AccountBook'][$this->hashCode()] = true;
        $keys = AccountBookTableMap::getFieldNames($keyType);
        $result = array(
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\AppBundle\Database\Propel\Model\AccountBook
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\AppBundle\Database\Propel\Model\AccountBook The current object, for fluid interface
     */
    ...
    {
        $criteria = new Criteria(AccountBookTableMap::DATABASE_NAME);

        if ($this->isColumnModified(AccountBookTableMap::COL_ACCOUNT_ID)) {
            $criteria->add(AccountBookTableMap::COL_ACCOUNT_ID, $this->account_id);
        }
        if ($this->isColumnModified(AccountBookTableMap::COL_BOOK_ID)) {
            $criteria->add(AccountBookTableMap::COL_BOOK_ID, $this->book_id);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \AppBundle\Database\Propel\Model\AccountBook (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \AppBundle\Database\Propel\Model\AccountBook Clone of current object.
     * @throws PropelException
     */
    ...
    public function getAccount(ConnectionInterface $con = null)
    {
        if ($this->aAccount === null && ($this->account_id != 0)) {
            ...
            /* The following can be used additionally to
                ...
                $this->aAccount->addAccountBooks($this);
             */
        }
    ...
    public function getBook(ConnectionInterface $con = null)
    {
        if ($this->aBook === null && ($this->book_id != 0)) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aBook->addAccountBooks($this);
             */
        }
    ...
    {
        if (null !== $this->aAccount) {
            $this->aAccount->removeAccountBook($this);
        }
        if (null !== $this->aBook) {
            $this->aBook->removeAccountBook($this);
        }
    ...
    {
        return (string) $this->exportTo(AccountBookTableMap::DEFAULT_STRING_FORMAT);
    }
