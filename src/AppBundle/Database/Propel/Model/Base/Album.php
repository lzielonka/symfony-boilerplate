...
abstract class Album implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\AppBundle\\Database\\Propel\\Model\\Map\\AlbumTableMap';
    ...
    /**
     * @var        \AppBundle\Database\Propel\Collection\ObjectCollection|ChildSong[] Collection to store aggregation of ChildSong objects.
     */
    protected $collSongs;
     ...
     * @var \AppBundle\Database\Propel\Collection\ObjectCollection|ChildSong[]
     */
    protected $songsScheduledForDeletion = null;

    /**
     * Initializes internal state of AppBundle\Database\Propel\Model\Base\Album object.
     */
     ...
     * @return $this|\AppBundle\Database\Propel\Model\Album The current object (for fluent API support)
     */
    public function setAlbumId($v)
    {
        ...
        if ($this->album_id !== $v) {
            $this->album_id = $v;
            $this->modifiedColumns[AlbumTableMap::COL_ALBUM_ID] = true;
        }
    ...
    public function setArtistId($v)
    {
        ...
        if ($this->artist_id !== $v) {
            $this->artist_id = $v;
            $this->modifiedColumns[AlbumTableMap::COL_ARTIST_ID] = true;
        }
    ...
    public function setTitle($v)
    {
        ...
        if ($this->title !== $v) {
            $this->title = $v;
            $this->modifiedColumns[AlbumTableMap::COL_TITLE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : AlbumTableMap::translateFieldName('AlbumId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->album_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : AlbumTableMap::translateFieldName('ArtistId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->artist_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : AlbumTableMap::translateFieldName('Title', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = AlbumTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\AppBundle\\Database\\Propel\\Model\\Album'), 0, $e);
        }
    }
    ...
    {
        if ($this->aArtist !== null && $this->artist_id !== $this->aArtist->getArtistId()) {
            $this->aArtist = null;
        }
    } // ensureConsistency
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->songsScheduledForDeletion !== null) {
                if (!$this->songsScheduledForDeletion->isEmpty()) {
                    \AppBundle\Database\Propel\Model\SongQuery::create()
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(AlbumTableMap::COL_ALBUM_ID)) {
            $modifiedColumns[':p' . $index++]  = 'album_id';
        }
        if ($this->isColumnModified(AlbumTableMap::COL_ARTIST_ID)) {
            $modifiedColumns[':p' . $index++]  = 'artist_id';
        }
        if ($this->isColumnModified(AlbumTableMap::COL_TITLE)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\AppBundle\Database\Propel\Model\Album
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\AppBundle\Database\Propel\Model\Album The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AlbumTableMap::COL_ALBUM_ID)) {
            $criteria->add(AlbumTableMap::COL_ALBUM_ID, $this->album_id);
        }
        if ($this->isColumnModified(AlbumTableMap::COL_ARTIST_ID)) {
            $criteria->add(AlbumTableMap::COL_ARTIST_ID, $this->artist_id);
        }
        if ($this->isColumnModified(AlbumTableMap::COL_TITLE)) {
            $criteria->add(AlbumTableMap::COL_TITLE, $this->title);
        }
    ...
    /**
     * Generic method to set the primary key (album_id column).
     *
     ...
     * objects.
     *
     * @param      object $copyObj An object of \AppBundle\Database\Propel\Model\Album (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \AppBundle\Database\Propel\Model\Album Clone of current object.
     * @throws PropelException
     */
    ...
    public function initSongs($overrideExisting = true)
    {
        ...
        $this->collSongs->setModel('\AppBundle\Database\Propel\Model\Song');
    }
     ...
     * @param      ConnectionInterface $con optional connection object
     * @return \AppBundle\Database\Propel\Collection\ObjectCollection|ChildSong[] List of ChildSong objects
     * @throws PropelException
     */
